...
function geom_plot!(fig, section_plot, steering, suspension)
    ...
    section_plot.ax_geom = GLMakie.Axis3(fig[row, col],
                                            ...
                                            ylabel = "y in [mm]",
                                            zlabel = "z in [mm]", 
                                            title = geometry_title(steering.ϕx, steering.ϕy, steering.ϕz, suspension),
...
"""
    ...
    Initializes and renders a 2D plot of the turning radius versus φz (steering angle around z-axis).

    # Arguments
    - `fig`: A `GLMakie.Figure` where the plot will be drawn.
    - `section_plot`: A `PlotSection` object that holds plot axes and observables.
    - `ϕ_max`: A tuple `(φx_max, φy_max, φz_max)` defining the steering angle limits.
    ...
    # Description
    This function:
    - Sets up a 2D `Axis` for plotting radius values over a sweep of φz (0 to φz_max).
    - Initially computes the turning radius using `steering_radii_φz(...)` with φx = 1, φy = 0.
    - Stores the results in an `Observable`, allowing reactive updates later.
    - Draws the radius curve over the φz range using `lines!`.
...
"""
function radii_plot!(fig,section_plot, ϕ_max, chassis, steering, suspension)
    ...
    # Limits
    GLMakie.xlims!(section_plot.ax_radii, 0, ϕz_max)
    ...
    ############| Radii ϕz Observervar 
    section_plot.obs_radii_ϕz = Observable(radii_ϕz)


    ############| Radii ϕz Ploting  
    xs = [ϕz for ϕz in 0.0:1.0:ϕz_max] 

    GLMakie.lines!(section_plot.ax_radii, xs, section_plot.obs_radii_ϕz)
end
...
Generates a 2D plot of the Ackermann ratio across a range of φz values and highlights the minimum and maximum ratio.
...
- `ϕ_max`: A tuple `(φx_max, φy_max, φz_max)` defining the range limits of the steering angles.
...
- Computes the Ackermann ratio for φz from `0` to `φz_max` at fixed `φx=0`, `φy=0`.
...
"""
function ratio_plot!(fig,section_plot, ϕ_max, chassis, steering, suspension)
    ...
    section_plot.ax_ratio = GLMakie.Axis(fig[row, col], 
                                            ...
                                            title = ackermann_ratio_title(ϕx, ϕy, ϕz, suspension), 
                                            titlesize = plot_title_size(),
                                            xticks = 0:5:40,
                                            yticks = 30:5:100)
    ...
    # Limits
    GLMakie.xlims!(section_plot.ax_ratio, 0, ϕz_max)
    GLMakie.ylims!(section_plot.ax_ratio, 30, 105)

    ############| Ackermannratio Data

    ratio = ackermannratio_ϕz(ϕx,ϕy,ϕz_max,chassis, steering, suspension)

    ############| Ackermannratio Observervar 

    section_plot.obs_ratio_ϕz = Observable(ratio)
    ...
    GLMakie.lines!(section_plot.ax_ratio, xs, section_plot.obs_ratio_ϕz)
    ...
    on(section_plot.obs_ratio_ϕz) do values
        set_ratio_ylims!(section_plot.ax_ratio, values)
    end

end

function ratio_ϕx_plot!(fig, section_plot, ϕ_max, chassis, steering, suspension)
    ...
    section_plot.ax_ratio_ϕx = GLMakie.Axis(fig[row, col],
                                            xlabel = "φx in [°]",
                                            ylabel = "Ackermann ratio [%]",
                                            title = ackermann_ratio_ϕx_title(ϕy, ϕz, suspension),
                                            titlesize = plot_title_size(),
                                            xticks = 0:5:ϕx_max,
                                            yticks = 30:5:100)

    set_axis_visible!(section_plot.ax_ratio_ϕx, false)

    GLMakie.xlims!(section_plot.ax_ratio_ϕx, 0, ϕx_max)
    GLMakie.ylims!(section_plot.ax_ratio_ϕx, 30, 105)

    ratio = ackermannratio_ϕx(ϕx_max, ϕy, ϕz, chassis, steering, suspension)

    section_plot.obs_ratio_ϕx = Observable(ratio)
    section_plot.obs_ratio_ϕx_min = Observable(finite_minimum(ratio))
    section_plot.obs_ratio_ϕx_max = Observable(finite_maximum(ratio))
    set_ratio_ylims!(section_plot.ax_ratio_ϕx, ratio)

    xs = [ϕx for ϕx in 0.0:1.0:ϕx_max]

    GLMakie.lines!(section_plot.ax_ratio_ϕx, xs, section_plot.obs_ratio_ϕx)
    GLMakie.hlines!(section_plot.ax_ratio_ϕx, section_plot.obs_ratio_ϕx_min, linestyle = :dash, color = :orange)
    GLMakie.hlines!(section_plot.ax_ratio_ϕx, section_plot.obs_ratio_ϕx_max, linestyle = :dash, color = :red)

    section_plot.txt_ratio_ϕx_min = GLMakie.text!(section_plot.ax_ratio_ϕx,
                                                    Point(max(ϕx_max - 10, 0), 55),
                                                    text = "Min → $(round(section_plot.obs_ratio_ϕx_min[], digits=2))%",
                                                    align = (:left, :bottom),
                                                    color = :orange)

    section_plot.txt_ratio_ϕx_max = GLMakie.text!(section_plot.ax_ratio_ϕx,
                                                    Point(max(ϕx_max - 5, 0), 55),
                                                    text = "Max → $(round(section_plot.obs_ratio_ϕx_max[], digits=2))%",
                                                    align = (:left, :bottom),
                                                    color = :red)

    lift(section_plot.obs_ratio_ϕx_min) do val
        section_plot.txt_ratio_ϕx_min.text = "Min → $(round(val, digits=2))%"
    end

    lift(section_plot.obs_ratio_ϕx_max) do val
        section_plot.txt_ratio_ϕx_max.text = "Max → $(round(val, digits=2))%"
    end

    on(section_plot.obs_ratio_ϕx) do values
        set_ratio_ylims!(section_plot.ax_ratio_ϕx, values)
    end
...
Creates a 3D surface plot of the Ackermann ratio over φx and φz steering angles.
...
- `ϕ_max`: A tuple `(φx_max, φy_max, φz_max)` defining the maximum steering angles.
...
- Computes the Ackermann ratio across a meshgrid of φx and φz values using `ackermannratio_surface(...)`, keeping φy fixed.
...
"""
function ratio_surface_plot!(fig,section_plot, ϕ_max, chassis, steering, suspension)
    ϕx_max, ϕy_max, ϕz_max = ϕ_max
    ...
    section_plot.ax_ratio_surface = GLMakie.Axis3(fig[row, col],
                                                    ...
                                                    zlabel = "ratio in [%]",
                                                    zticks = 50:10:100, 
                                                    title = ackermann_ratio_surface_title(suspension),
                                                    titlesize = plot_title_size(),) #
    ...
    # Limits
    GLMakie.xlims!(section_plot.ax_ratio_surface, 0, ϕx_max)
    GLMakie.ylims!(section_plot.ax_ratio_surface, 0, ϕz_max)
    GLMakie.zlims!(section_plot.ax_ratio_surface, 50, 105)

    ############| Ackermannratio Data

    ratio_surface = ackermannratio_surface(chassis, steering, suspension, (ϕx_max,ϕy,ϕz_max))
...
function deviation_plot!(fig,section_plot, ϕ_max, chassis, steering, suspension)
    ...
    # Limits
    GLMakie.xlims!(section_plot.ax_deviation, 0, ϕz_max)
    GLMakie.ylims!(section_plot.ax_deviation, -500,500 )

    ############| Ackermannratio Data

    deviation = ackermann_deviation_ϕz(ϕx,ϕy,ϕz_max,chassis, steering, suspension)

    ############| Ackermannratio Observervar 

    section_plot.obs_deviation_ϕz = Observable(deviation)
    ...
    GLMakie.lines!(section_plot.ax_deviation, xs, section_plot.obs_deviation_ϕz)
...
function deviation_surface_plot!(fig,section_plot, ϕ_max, chassis, steering, suspension)
    ϕx_max, ϕy_max, ϕz_max = ϕ_max
    ...
    # Limits
    GLMakie.xlims!(section_plot.ax_deviation_surface, 0, ϕx_max)
    GLMakie.ylims!(section_plot.ax_deviation_surface, 0, ϕz_max)
...
"""
    update_geometry!(ϕ, section_plot, steering, suspension)

Updates the visual representation of the steering geometry based on the given steering angles.

# Arguments
- `ϕ`: A tuple `(φx, φy, φz)` representing the current steering angles.
...
"""
function update_geometry!(ϕ, section_plot, steering, suspension)
...
"""
    ϕ_vs_δ_plot!(fig, section_plot, ϕ_max, steering)
...
- `ϕ_max`: A tuple `(φx_max, φy_max, φz_max)` representing the upper bounds of steering input angles.
...
- Uses `ax_ϕ_vs_δi` and `ax_ϕ_vs_δo` to compute the inner and outer wheel steering angles across a range of `φx` and `φz`.
...
"""
function ϕ_vs_δ_plot!(fig, section_plot, ϕ_max, steering, suspension)
    ϕx_max, ϕy_max, ϕz_max = ϕ_max
    ...
    section_plot.ax_ϕ_vs_δ_surface = GLMakie.Axis3(fig[row, col],
                                            xlabel = "φx in [°]", 
                                            ylabel = "φz in [°]",
    ...
    set_axis_visible!(section_plot.ax_ϕ_vs_δ_surface,    false)

    # Limits
    GLMakie.xlims!(section_plot.ax_ϕ_vs_δ_surface, 0, ϕx_max)
    GLMakie.ylims!(section_plot.ax_ϕ_vs_δ_surface, 0, ϕz_max)
    GLMakie.zlims!(section_plot.ax_ϕ_vs_δ_surface, 0, 105)
    ...
    ############| Ackermannratio Observervar 
    section_plot.obs_ϕ_vs_δi_surface = Observable(ϕ_vs_δi_surface)
    section_plot.obs_ϕ_vs_δo_surface = Observable(ϕ_vs_δo_surface)

    ############| Ackermannratio Ploting  

    GLMakie.surface!(
        section_plot.ax_ϕ_vs_δ_surface,
        0.0:1.0:ϕx_max,
        0.0:1.0:ϕz_max,
        section_plot.obs_ϕ_vs_δi_surface;
    ...
    GLMakie.surface!(
        ...
        0.0:1.0:ϕx_max,
        0.0:1.0:ϕz_max,
        section_plot.obs_ϕ_vs_δo_surface;
...
function left_wheel_delta_plot!(fig, section_plot, ϕ_max, steering, suspension)
    ϕx_max, ϕy_max, ϕz_max = ϕ_max
    ...
    GLMakie.ylims!(section_plot.ax_left_wheel_delta, 0, ϕz_max)
...
function wheel_center_surface_plot!(fig, section_plot, ϕ_max, steering, suspension)
    ϕx_max, ϕy_max, ϕz_max = ϕ_max
...
function roll_kinematics_plot!(fig, section_plot, chassis, steering, suspension)
    ...
    ϕ = (steering.ϕx, steering.ϕy, steering.ϕz)
    (
        roll_values,
        left_camber,
        right_camber,
    ...
    ) = roll_kinematics(ϕ, chassis, steering, suspension)
