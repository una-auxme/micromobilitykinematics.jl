...
"""
    tie_rod_track_lever_example_steering()
...
"""
    tie_rod_track_lever_example_suspension()

Creates the same suspension geometry as `example.jl`.
"""
function tie_rod_track_lever_example_suspension()
    lower_wishbones = (
        LowerWishbone(
            id = :Left,
...
tie_rod_track_lever_unit(v) = v ./ norm(v)

function tie_rod_track_lever_angle_between_deg(a, b)
...
function local_track_lever_basis(track_lever_mount_to_joint; vehicle_z = [0.0, 0.0, 1.0])
    x_axis = tie_rod_track_lever_unit(track_lever_mount_to_joint)
    ...
    z_axis = tie_rod_track_lever_unit(z_candidate)
    y_axis = tie_rod_track_lever_unit(cross(z_axis, x_axis))
...
function tie_rod_track_lever_geometry(steering, side_index)
    ...
    track_lever_mount_to_joint = joint .- mount
    track_lever_joint_to_mount = mount .- joint
    tie_rod_joint_to_sphere = sphere .- joint

    basis = local_track_lever_basis(track_lever_mount_to_joint)

    return (
        track_lever_length_mm = norm(track_lever_mount_to_joint),
        angle_deg = tie_rod_track_lever_angle_between_deg(track_lever_joint_to_mount, tie_rod_joint_to_sphere),
        tie_rod_local = to_local(tie_rod_joint_to_sphere, basis),
    )
end
...
mutable struct TieRodTrackLeverSummary
    side::Symbol
    side_index::Int
    ...
    plotted_count::Int
    track_lever_length_mm::Float64
end

function TieRodTrackLeverSummary(side, side_index, reference_direction, track_lever_length_mm)
    return TieRodTrackLeverSummary(
        side,
        side_index,
        Inf,
        -Inf,
        ...
        Dict{String,Int}(),
        0,
        track_lever_length_mm,
    )
end

function update_summary!(summary, geometry, pose)
    direction = tie_rod_track_lever_unit(geometry.tie_rod_local)
    reference_deviation = tie_rod_track_lever_angle_between_deg(direction, summary.reference_direction)
...
function perpendicular_basis(axis)
    ...
    first_axis = tie_rod_track_lever_unit(cross(axis, helper))
    second_axis = tie_rod_track_lever_unit(cross(axis, first_axis))
...
function local_refine_neutral_direction(initial_axis, directions; step_deg = 20.0, min_step_deg = 0.02)
    best_axis = tie_rod_track_lever_unit(initial_axis)
    ...
    while step >= min_step_deg
        ...
        for azimuth in range(0.0, 360.0; length = 25)[1:end-1]
            ...
            candidate_axis = tie_rod_track_lever_unit(best_axis .* cosd(step) .+ tangent_direction .* sind(step))
...
function optimal_neutral_direction(directions, reference_direction, direction_sum)
    ...
    norm(direction_sum) > eps(Float64) && push!(candidates, tie_rod_track_lever_unit(direction_sum))
    ...
    norm(pair_sum) > eps(Float64) && push!(candidates, tie_rod_track_lever_unit(pair_sum))
...
function optimal_axis_component_basis(optimal_axis; local_z = [0.0, 0.0, 1.0])
    axis = tie_rod_track_lever_unit(optimal_axis)
    ...
    vertical = tie_rod_track_lever_unit(vertical_candidate)
    lateral = tie_rod_track_lever_unit(cross(vertical, axis))
...
function finalize_summary!(summary)
    ...
    summary.optimal_reference_offset_deg = tie_rod_track_lever_angle_between_deg(
        optimal_direction,
        summary.reference_direction,
    )
    ...
    for (direction, pose) in zip(summary.directions, summary.direction_poses)
        angle = tie_rod_track_lever_angle_between_deg(optimal_direction, direction)
...
function add_plot_vector!(
    ...
    tie_rod_plot_vector = normalize_tie_rod_vectors_for_plot ?
                          tie_rod_track_lever_unit(tie_rod_local) .* display_tie_rod_length_mm :
                          tie_rod_local
    start_point = Point3f(summary.track_lever_length_mm, 0.0, 0.0)
    end_point = Point3f((summary.track_lever_length_mm .+ tie_rod_plot_vector[1]), tie_rod_plot_vector[2], tie_rod_plot_vector[3])
...
For the 3D plot the track lever is kept fixed in a local coordinate system:

- origin at the track lever mount
...
function tie_rod_track_lever_angle_sweep(theta_limits_deg,
    ...
    summary = TieRodTrackLeverSummary(
        side,
        side_index,
        tie_rod_track_lever_unit(reference.tie_rod_local),
        reference.track_lever_length_mm,
    )
...
function print_tie_rod_track_lever_summary(result)
    println()
    println("Tie rod / track lever angle range")

    summary = result.summary
    mean_direction = tie_rod_track_lever_unit(summary.direction_sum)
    ...
    @printf(
        "  mean tie-rod direction in local track-lever frame: [%.4f, %.4f, %.4f]\n",
        mean_direction[1],
        mean_direction[2],
        mean_direction[3],
    )
    @printf(
        "  optimal joint neutral axis in local track-lever frame: [%.4f, %.4f, %.4f]\n",
...
function plot_tie_rod_track_lever_vectors(result;
    ...
    ax = Axis3(
        fig[1:2, 1],
        xlabel = "local x along track lever [mm]",
        ylabel = "local y [mm]",
    ...
    track_end = Point3f(summary.track_lever_length_mm, 0.0, 0.0)
    lines!(ax, [track_start, track_end]; color = :black, linewidth = 5)
    scatter!(ax, [track_start, track_end]; color = :black, markersize = 10)
    ...
    optimal_end = Point3f(
        summary.track_lever_length_mm + summary.optimal_direction[1] * optimal_vector_length,
        summary.optimal_direction[2] * optimal_vector_length,
        summary.optimal_direction[3] * optimal_vector_length,
    )
    lines!(ax, [track_end, optimal_end]; color = :red, linewidth = 6)
    ...
    Colorbar(fig[1:2, 2], colormap = :viridis, limits = colorrange, label = "angle between tie rod and track lever [deg]")
    Label(
        fig[2, 3],
...
function run_tie_rod_track_lever_angle_example()
    ...
    steering_to_evaluate = isdefined(Main, :steering) ? deepcopy(Main.steering) : tie_rod_track_lever_example_steering()
